...
## Example skill complexity matrix
...
| Architecture memory  | `accumulate-knowledge-arch`    | none              | 0                              | no   | Low — LLM-only + Mem0 + shared dept namespace                               | Low — SKILL.md + `context-strategy: dept-memory` + seed data                  | Single Mem0 `entityId` **`dept-architecture`** for team PoC |

**Memory namespaces:** Skills using **`fixture-enrichment`** set `memory-entity-domain: journey` or `legacy` in `SKILL.md`; with `MEMORY_ENABLED=true`, the API resolves **`entityId`** like `user-5001-journey` vs `user-5001-legacy`. The **`accumulate-knowledge-arch`** skill uses **`context-strategy: dept-memory`** and a fixed **`entityId`** **`dept-architecture`** (see `SHARED_DEPT_ARCHITECTURE_ENTITY_ID` in `@agent-harness/memory`) so seeded and live turns share one department namespace.
